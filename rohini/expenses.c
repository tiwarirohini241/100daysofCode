...
void addexpense()
{
    ...
    printf("Enter date(DD-MM-YYYY),category ,amount and description:\n");
    scanf("%s %s %d %s",exp.date,exp.group,&exp.price,exp.description);
    fprintf(fp,"%s %s %d %s\n",exp.date, exp.group, exp.price, exp.description);
    fclose(fp);
...
void viewexpense()
{
    ...
    while(fscanf(fp,"%s %s %d %s",exp.date, exp.group, &exp.price, exp.description) != EOF)
    {
        printf("%s %s %d %s\n",exp.date, exp.group, exp.price, exp.description);
    }
    fclose(fp);
...
void filterexpense()
{
    ...
    while(fscanf(fp,"%s %s %d %s",
        exp.date, exp.group, &exp.price, exp.description) != EOF)
    {
        if (strcmp(exp.group, target)==0)
        {
            printf("%s %s %d %s\n",exp.date, exp.group, exp.price, exp.description);
            found = 1;
        }
